...
后续与 Claude Code 对话时，统一通过 `memory/wiring_map.yaml` 解析你说的 Q1/Q2/CZ/读出含义。

### 规则

- 你若只说 `Q1` 或 `Q2`，默认解释为该比特的 `0-1激发` 驱动通道。
- 你若明确写 `Q1 1-2激发`、`Q2 1-2激发`，优先按显式跃迁通道解释。
- 两比特操作（例如 `CZ`）在使用上按“驱动通道”处理，映射逻辑与单比特驱动一致。
- 读出通道拆分为 `output_drive_channel`（读出激励）和 `acquisition_channel`（采集）。
- 多个比特可共享一个读出通道，但每个比特必须填写各自 `per_qubit_readout_freq_hz`。
- 所有硬件端口、衰减、滤波、频率由你手填，脚本只读取该 YAML，不再硬编码语义。

### 最小填写要求

- 在 `drive_channels` 中填写每个逻辑驱动（可包含同一比特多个跃迁）。
- 在 `readout_channels` 中填写共享关系与输入/采集通道。
- 若未来涉及 DC 偏置，需填写 `safety.max_safe_voltage`。
